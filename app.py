...
def check_answers(answers):
    ...
    keyword_dict = {
        'answer4': ['объединять', 'единую сеть'],
        'answer7': ['алгоритмы'],
        'answer12': ['дополненная реальность']
    }

    results = {}
    for question, user_answer in answers.items():
        if question in keyword_dict:
            keywords = keyword_dict[question]
            contains_keywords = all(keyword in user_answer.lower() for keyword in keywords)
            results[question] = contains_keywords
